...
static void _node(const char* processName, const char* userScript) {
...

  {
    ...
    std::unique_ptr<v8::Platform>  platform_= v8::platform::NewDefaultPlatform();// CreateDefaultPlatform();  //v8_default_thread_pool_size = 4;
    ...
    bool more;
    do {
      platform::PumpMessageLoop(platform_.get(), isolate_);
